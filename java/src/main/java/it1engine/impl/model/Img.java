...
public class Img implements ImgInterface {
     ...
     */
    // @Override
    // public void drawOn(ImgInterface board, int cellX, int cellY, int cellW, int
    // cellH) {
    ...
    // board.getBufferedImage().getWidth(), board.getBufferedImage().getHeight());
    ...
    // public void drawAt(ImgInterface target, int xPix, int yPix, int wPix, int
    // hPix) {
    // if (img == null || target.getBufferedImage() == null)
    // throw new IllegalStateException("Both images must be loaded.");
    // Image tmp = img.getScaledInstance(wPix, hPix, Image.SCALE_SMOOTH);
    // BufferedImage resized = new BufferedImage(wPix, hPix,
    // BufferedImage.TYPE_INT_ARGB);
    // Graphics2D g2 = resized.createGraphics();
    // g2.drawImage(tmp, 0, 0, null);
    // g2.dispose();
    ...
    public void show() {
        ...
        // This is a simplified version - in a real application you might want more
        // control
        try {
